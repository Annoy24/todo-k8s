...
router.post('/createuser',[                         //validation array
    ...
    // res.send(req.body)
    //pasted from express validator

    try{
    //created an user and checks if email is unique
    let userMail = await User.findOne({email : req.body.email})
    if(userMail){
        return res.status(400).send({success,error : "please enter a unique email"})
    }
    ...
    let user = await User.create({
      ...
      //  since er are using async await hece we donot need these .then
      //   .then(user => res.json(user))
      //   .catch(err=>{
      //     console.log(err)
      //     res.send({error:"enter unique email",message:err.message})});
      
      //since we donot want to send user his data only, we will send the auth token which will be used 
      //to verify user for the next time
...
router.post('/loginuser',[                         //validation array
    body('email').isEmail(),
    ...
    try{
        ...
        if(!user){
            res.status(400).send({success,message:"Login with proper credentials"})
        }
